...
describe('homepage operations board', () => {
  ...
  it('shows invoice payment summary from trustworthy ACTUAL_IN cashflow metadata and falls back when unavailable', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([
      {
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Payment summary');
    expect(cashflowSummary).toHaveTextContent('Total incl. VAT: CZK 121,000.00');
    expect(cashflowSummary).toHaveTextContent('Paid: CZK 60,000.00');
    expect(cashflowSummary).toHaveTextContent('Remaining: CZK 61,000.00');
    expect(cashflowSummary).toHaveTextContent('Payment state: Partially paid');
  ...
  it('shows compact invoice party summary only from trustworthy display names and falls back otherwise', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Party summary');
    expect(cashflowSummary).toHaveTextContent('Buyer: Acme Interiors s.r.o.');
    expect(cashflowSummary).toHaveTextContent('Supplier: Planovna Studio s.r.o.');
    expect(cashflowSummary).toHaveTextContent('No trustworthy customer-facing name is available.');
    expect(cashflowSummary).not.toHaveTextContent('inv-2');
  });
  ...
  it('shows invoice note/reference summary from one trustworthy invoice-level field and falls back otherwise', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
      {
        ...
        hasBreakdown: true,
        currency: 'EUR',
        businessContext: 'Installation milestone A',
        pdfPath: '/invoices/inv-1/pdf',
      },
      {
        id: 'inv-2',
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Business context');
    expect(cashflowSummary).toHaveTextContent('Note / reference: Installation milestone A');
    expect(cashflowSummary).toHaveTextContent('No trustworthy customer-facing note or reference is available.');
    expect(cashflowSummary).not.toHaveTextContent('inv-2');
  });

  it('shows explicit invoice document status in the identity area and falls back when unavailable', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Document status');
    expect(cashflowSummary).toHaveTextContent('Document status: ISSUED');
    expect(cashflowSummary).toHaveTextContent('Document status is not reliably available.');
    expect(cashflowSummary).toHaveTextContent('Payment summary');
    expect(cashflowSummary).toHaveTextContent('Due summary');
  });

  it('shows invoice origin from one safe invoice-level source field and falls back otherwise', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Invoice origin');
    expect(cashflowSummary).toHaveTextContent('Source / type: Manual invoice');
    expect(cashflowSummary).toHaveTextContent('Invoice origin is not reliably available.');
  ...
  it('shows invoice language only from one safe invoice-level locale field and falls back otherwise', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Invoice language');
    expect(cashflowSummary).toHaveTextContent('Language / localization: German');
    expect(cashflowSummary).toHaveTextContent('Invoice language is not reliably available.');
  ...
  it('shows explicit supplier tax-id fallback when trustworthy invoice-level metadata is unavailable', async () => {
    const client = createClient();
    client.auth.login.mutate.mockResolvedValue({ accessToken: 'token-owner' });
    client.cashflow.list.query.mockResolvedValue([]);
    client.invoice.list.query.mockResolvedValue([
    ...
    const cashflowSummary = await screen.findByRole('region', { name: 'Cashflow summary' });
    expect(cashflowSummary).toHaveTextContent('Supplier tax ID');
    expect(cashflowSummary).toHaveTextContent('Supplier tax ID: CZ12345678');
